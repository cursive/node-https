...
var SwipeView = (function(window, document) {
	...
	SwipeView.prototype = {
		...
		refreshSize: function() {
			...
			this.snapThreshold = this.options.snapThreshold === null ? Math.round(this.pageWidth * 0.15) :
				/%/.test(this.options.snapThreshold) ? Math.round(this.pageWidth * this.options.snapThreshold.replace('%', '') / 100) : this.options.snapThreshold;
		},
		...
		__move: function(e) {
			...
			if (!this.directionLocked && this.stepsY > this.stepsX) {
				...
				//Dan added scrolle nalebe ldisable
				//myScroll.enable();
				return;
			}
			//DDan disable iscrll
			...
			//dan added to give percentage scroll (320/100) for iphone portrait
			// hoverIntro(dist / 3.2);
		...
		__end: function(e) {
			...
			if (dist < this.snapThreshold) {
				...
				this.__pos(-this.page * this.pageWidth);
				//Daniel add szipe cannceeled function
				// swipeCancelled();
				return;
			}
